...
if not df_comuna.empty:
    ...
    st.pyplot(fig)
    #st.markdown("<div style='text-align:right; color:red;'>Métricas comunales anuales</div>", unsafe_allow_html=True)
else:
    st.info("No hay datos para la selección actual.")

# KPIs 
#st.markdown("<h2 style='text-align:center;'>Lenguaje</h2>", unsafe_allow_html=True)
if not df_comuna.empty:
...
#with col1:
   # st.metric("Año", agno_max_leng)
   # st.metric("Comuna", max_leng)
#with col2:
   # st.metric("Año", agno_min_leng)
    #st.metric("Comuna", min_leng)


#st.markdown("<h2 style='text-align:center;'>Matemáticas</h2>", unsafe_allow_html=True)
...
#with col3:
   # st.metric("Año", agno_max_mat)
   # st.metric("Comuna (max)", max_mat)
#with col4:
  #  st.metric("Año", agno_min_mat)
   # st.metric("Comuna (min)", min_mat)

#col3.metric(f"Año: {agno_max_mat}", max_mat, comuna_sel)
#col4.metric(f"Año: {agno_min_mat}", min_mat, f"-{comuna_sel}")

#st.markdown("<div style='text-align:center; color:red;'>Métricas regionales anuales</div>", unsafe_allow_html=True)